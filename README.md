...
- **Send Phone/App data to the boat network** continuously publishes every currently valid locally-owned measurement, such as phone GNSS, aligned phone heading, confirmed Trip motion, pressure and explicitly App-derived wind. It never echoes instruments received from the boat and never yields merely because another device reports the same field; receiving instruments own source selection. The authoritative route reuses the existing full-duplex input socket; separately configured gateway TCP/UDP routes are explicit advanced options.
- **KC-2W hardware note:** use 38400 baud on the NMEA 0183 side for the complete Phone/App feed. Vessel testing confirmed that 4800 baud became the bottleneck after Raymarine joined, causing intermittent DataList fields and `NO HEADING`; changing only the KC-2W rate to 38400 resolved it.
...
- Phone heel/pitch is optional and belongs to Trip Watch: place the screen plane parallel to the vessel reference plane, select the bow-facing edge and confirm. This maps axes without falsely zeroing a vessel that is already heeled. Before picking the phone up, pause attitude; replace and confirm it to start a new valid segment. Phone pressure remains independent.
...
- Completed trips have a bounded-memory report covering the recorded sailing track, SOG/BSP, fastest 500 m, synchronized speed-versus-heel bands, conservative point-of-sail/tack/gybe observations, motion, depth/UKC, wind, pressure, source changes, events and waypoints. Isolated short attitude spikes may be excluded only in report statistics; sustained heel, gradual changes and GPS-evidenced turns remain. Replay can colour the route by SOG, BSP, heel, TWS, AWS, motion or depth and jump directly to events.
...
The two NMEA products are intentionally independent. Phone/App boat output continuously sends all currently valid locally-owned values and normally writes through the existing full-duplex connection; dedicated gateway TCP/UDP is available only as an explicit advanced route. The phone-hosted NMEA service serves the same kind of Phone/App-owned feed to other devices through its own listener and never forwards the Boat input stream. Receiving equipment—not a hidden App backup policy—chooses its preferred source. Phone Position Output remains hard-conflicted with using Boat NMEA position as the App GPS source.
