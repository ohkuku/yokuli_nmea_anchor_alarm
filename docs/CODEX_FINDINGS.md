...
- Root cause: HorizontalPager precomposes the adjacent page before it becomes the settled viewport, while the real map preview inside Overview legitimately consumes gestures that begin over it. The test sent a centre swipe into that map, then treated existence of the precomposed Sailing page as proof that the pager moved. The route test rendered a report sub-composable in `createComposeRule`'s bare host and treated the host Activity's foreground-window heuristic as product visibility; affected emulator shards reported the measured Card as not displayed. Unit, Lint and Assemble steps had no individual `timeout-minutes`, so a stuck Gradle child was not bounded at the failing operation.
- Failing test: the two GitHub check-run annotations above; the workflow hang is run 70 `Android lint`.
- Fix commit: **Current `codex/develop` delivery commit**
- Verification result: **The Sail page picker and every destination now expose stable semantics tags. The test navigates through that real accessible UI, then still requires all six instruments to be fully displayed. The empty-route test requires the tagged Card to exist with non-zero measured root bounds and the exact explanatory copy, without substituting the bare test host's foreground-window heuristic for product layout. Unit, Lint and Debug Assemble now have explicit 20/15/15-minute step timeouts; existing always-run failure-bundle upload remains intact. Android-test sources compile locally. Final remote shard verification is required after push.**
