...
- Severity: **P1 / NMEA ownership and operator control**
- User story: Phone/App → boat output fills measurements intentionally supplied by Boat Watch. Phone position must be off by default when the boat already has GPS, and the sailor must explicitly choose whether RMC/GGA/VTG/ZDA are published. Receiving or publishing Phone position must never make an independent Boat GPS invalid merely because both antennas are on the same vessel.
- Evidence: `OutputSettingsRepository.canonical()` and `NmeaDeviceOutputSettings.publisherConfiguration()` both overwrote `phonePositionEnabled=true` and `positionPolicy=ALWAYS`. `NmeaPublisherConfig` discarded the stream choice entirely, while `AnchorWatchNmeaPublisher.publishDue()` scheduled every stream and reported every decision as `ALWAYS`. There was no output-page control. Older builds therefore persisted `phone_position_enabled=true` even though the user had never opted in.
- Reproduction steps: configure Phone/App boat output without any per-stream choice, start TX and inspect generated/written data; valid Phone GNSS produces RMC/GGA/VTG/ZDA. Stop and reopen the page; there is no way to keep aligned heading/pressure output while suppressing Phone position.
- Root cause: the earlier product-feed simplification correctly removed Boat-data forwarding and BACKUP ownership, but incorrectly collapsed an operator publication choice into a forced canonical stream.
- Failing test: `NmeaDeviceOutputPolicyTest.phonePositionIsOffByDefaultAndCannotBeForcedOnByPublisherCanonicalization` and `AnchorWatchNmeaPublisherTest.validPhoneFixIsNotEncodedUntilPhonePositionPublicationIsExplicitlyEnabled`.
- Fix commit: **Current `codex/develop` delivery commit**
- Verification result: **A new-version opt-in key deliberately ignores the old forced-true preference. Phone position now remains OFF through persistence, publisher configuration, scheduling and encoding until the sailor enables the dedicated output-page switch. Toggling it during a running output session is serialized through the foreground runtime refresh. The independent Phone NMEA server remains a full local feed and explicitly includes Phone position. Boat RX continues to quarantine only byte-identical recent App TX frames; its production parse path does not call the approximate semantic matcher. Production, JVM-test and Android-test Kotlin sources compile. Tests were not executed in this pass by user instruction.**
- Real hardware verified: **No — verify with KC-2W/Raymarine that Phone-position OFF emits no RMC/GGA/VTG/ZDA, while Boat GPS remains accepted and Phone heading continues at 1 Hz.**
- Status: **FIXED IN CODE — PRODUCTION/JVM-TEST/ANDROID-TEST SOURCES COMPILE; TESTS NOT RUN; REAL-VESSEL QA PENDING**

## Finding P0-043 — Phone-hosted NMEA service disconnected after screen-off

- Severity: **P0 / explicitly running network service loses its clients in background**
- User story: after the sailor explicitly starts the Phone NMEA TCP service, its listener and 1 Hz local feed must continue while the screen is locked. Client loss must not stop the listener. Android process reclamation during the same device boot must restore the explicit lease; device reboot must still require another Start.
- Evidence: `RuntimeResourceManager` selected only `WIFI_MODE_FULL_LOW_LATENCY` on Android Q+, while Android documents that this lock is active only when the screen is on. The optional global `keepWifiAwake=false` path could also revoke `NMEA_SHARING`'s explicit Wi-Fi requirement. Independently, `LocalNmeaServerSettingsRepository` kept `serverRequested` only in a `MutableStateFlow`; a `START_STICKY` process recreation therefore restored the configuration with `serverRequested=false`. The runtime's initial uncalibrated sensor emission could then permanently clear any attempted restored lease.
- Reproduction steps: start Data → Output → Phone NMEA service, connect a dashboard client, confirm 1 Hz data, then lock the phone. On affected devices the client disconnects or the stream stops. If the process is reclaimed, reopen the App and observe that the configured listener is no longer running despite no explicit Stop.
- Root cause: the network power mode covered only screen-on, and the product's explicit run intent did not survive the exact process restart that `START_STICKY` promises callers must handle.
- Failing test: `LongRunningFaultStoryTest.keepWifiPreferenceAppliesImmediatelyOnlyToCurrentNetworkOwners` now requires the listener's mandatory lock to survive the optional preference being disabled; `NmeaSharingServerTest.explicitLocalServerLeaseRestoresOnlyWithinTheSameDeviceBoot` covers process-restart versus reboot semantics. Physical screen-off behavior is listed in `docs/PHYSICAL_SOAK_CHECKLIST.md`.
- Fix commit: **Current `codex/develop` delivery commit**
...
- Failing test: `P0RemediationPolicyTest.negativeAcceptedAgeIsReportedAsClockAheadNotStale`, `providerFixArrivingDuringArmIsReadyAtThePostPrimeDecisionClock`, `conditionFreshnessUsesItsPostSnapshotDecisionClock`; device regressions `newArmIntegrityEpochReplacesAnIdleStaleAcceptedCache` and `duplicatePrimeWithFreshAcceptedStateStillRemainsReady`; existing `freshSystemGpsArmCreatesAnActiveSessionWithoutNmea` and `freshNmeaGpsArmCreatesActiveSessionImmediately` remain the end-to-end stories.
- Fix commit: **Current `codex/develop` delivery commit**
...
## Finding P1-057 — Centre-learning progress was capped by the map render budget

- Severity: **P1 / misleading anchor-centre progress and restore contamination**
- User story: the Watch page and foreground notification must show the cumulative accepted positioning points in the current centre-analysis epoch. Limiting map rendering for performance must not make learning appear to stop, and restarting the process after a manual analysis reset must not restore evidence from the previous epoch.
- Evidence: `WatchPanel` displayed `state.points.size`, while `MainViewModel` deliberately queried only the latest 4,800 points for map rendering. `AnchorWatchRuntime.snapshot()` exposed the size of its bounded retained list, so its notification count could later fall after 8,000-point compaction. Runtime restore also loaded every session point into the estimator without applying `estimationEpochStartedAt`.
- Reproduction steps: keep a Back down centre-learning session active past 4,800 recorded points and compare the Watch page with the foreground notification/database. Then choose Start a new analysis epoch, restart the App and inspect estimator evidence.
- Root cause: render-trail size, cumulative epoch progress and bounded estimator-retention size were treated as one count. The restore path honored heading-evidence epoch events but omitted the centre-analysis timestamp boundary.
- Failing test: `P0OperabilityComposeTest.centreLearningProgressIsNotCappedByTheBoundedMapTrail`; Room query/source compilation covers the independent epoch count, while the runtime regression is covered by the monotonic counter and restore boundary implementation.
- Fix commit: **Current `codex/develop` delivery commit**
...
- Evidence: run 69 passed Unit/Lint/Debug and API 36 but failed shard 1 in `sailMfdKeepsCoreInstrumentsInTheFirstViewportAndCockpitReallyLocks` and shard 2 in `completedTripWithoutCoordinatesShowsAnExplicitRouteReason`. Run 70 then remained in `lintDebug` without reaching artifacts even though the workflow only had a 45-minute job timeout. Run 72 proved Unit/Lint/Assemble and API 36 green, then exposed two remaining hosted-emulator races: the menu-triggered Pager animation was observed mid-frame, the bare report fragment briefly had no attached semantics tree, and a service/DataStore migration exceeded an artificial five-second test deadline.
...
- Root cause: HorizontalPager precomposes the adjacent page before it becomes the settled viewport, while the real map preview inside Overview legitimately consumes gestures that begin over it. The original test sent a centre swipe into that map; the first correction still treated a partially entered animated destination as settled. The empty route also chained its `trip_route_empty` identity onto a Card whose caller already supplied `trip_route_map` or `open_trip_map_*`; collapsed Compose semantics retained one identity, making the visible reason inaccessible under the other. The legacy-share migration test crossed the foreground-service queue and two DataStore transactions but allowed only five seconds on a busy hosted emulator. Unit, Lint and Assemble steps had no individual `timeout-minutes`, so a stuck Gradle child was not bounded at the failing operation.
- Failing test: the two GitHub check-run annotations above; the workflow hang is run 70 `Android lint`.
- Fix commit: **Current `codex/develop` delivery commit**
- Verification result: **The Sail page picker and every destination expose stable semantics tags; explicit menu selection now changes pages immediately rather than leaving a transient half-page animation. The test still requires all six instruments to be fully displayed. The map container keeps its click identity while the empty explanation owns a distinct child semantics node. Its regression is no longer an isolated sub-composable assertion: it inserts a completed no-position trip, launches the real Activity, opens Sail → Trips → that trip, scrolls the route into view, and requires the visible explanation in the actual history viewport. The migration test still asserts the full stopped/no-socket outcome but allows the asynchronous persistence path 15 seconds. Unit, Lint and Debug Assemble have explicit 20/15/15-minute step timeouts; existing always-run failure-bundle upload remains intact. Android-test sources compile locally. Final remote shard verification is required after push.**
