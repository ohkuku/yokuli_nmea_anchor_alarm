...
- Fix commit: **Current `codex/develop` delivery commit**
- Verification result: **The Sail page picker and every destination expose stable semantics tags; explicit menu selection now changes pages immediately rather than leaving a transient half-page animation. The test still requires all six instruments to be fully displayed. The empty-route test supplies a full viewport, waits for one attached tagged Card, requires non-zero bounds and verifies the exact explanatory copy. The migration test still asserts the full stopped/no-socket outcome but allows the asynchronous persistence path 15 seconds. Unit, Lint and Debug Assemble have explicit 20/15/15-minute step timeouts; existing always-run failure-bundle upload remains intact. Android-test sources compile locally. Final remote shard verification is required after push.**
