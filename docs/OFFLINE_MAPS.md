...
```mermaid
flowchart TD
    G["Google Map / Satellite / styled Normal"] -->|"只由官方 SDK 在线读取"| M["Google Maps renderer"]
    O["OpenSeaMap seamarks · OpenStreetMap"] --> C["最近使用磁盘缓存"]
    L["LINZ Local depth · CC BY 4.0"] --> C
    C --> M
...
1. **Google 地图/卫星/清淡样式**：不预取、不拦截、不缓存、不导出。Google 官方政策限制未授权缓存和离线使用：[Map Tiles API policies](https://developers.google.com/maps/documentation/tile/policies)。
2. **OpenSeaMap / LINZ 最近使用区域**：两种来源各自只缓存 App 实际请求过的瓦片，不预取、不遍历航区。每个缓存目录上限 100 MB；7 天内直接使用，超过 7 天优先刷新，刷新失败则回退到旧 tile。
3. **用户导入 MBTiles**：设置 → 地图数据可导入 raster PNG/JPG/WEBP MBTiles。文件先复制到临时位置、验证 SQLite/tiles schema、zoom、图片签名和空间余量，成功后才原子替换应用私有副本。
...
OpenSeaMap seamark 瓦片按其 FAQ 标明的 CC BY-SA 2.0 使用并持续显示 `OpenSeaMap · OpenStreetMap`；底层 OpenStreetMap 数据署名保持可见。两者与 LINZ 使用独立缓存，不能因为某一来源允许缓存就推断其他来源也允许。
...
- “Clear rebuildable caches”会删除 OpenSeaMap/LINZ recent tiles、LINZ depth/tide cache 和派生声呐网格，然后从原始数据重建声呐网格。
- 用户导入的 `current.mbtiles` 不会被“清缓存”删除，只能在设置 → 地图数据明确点击 Remove。
- Storage 页面分别显示数据库、离线地图、临时 cache 和剩余空间。

### 已知限制

- Google renderer 本身完全离线时可能没有底图文字/陆地样式；LINZ 或 MBTiles overlay 仍可绘制已有覆盖区。
- 最近 OpenSeaMap/LINZ 缓存只包含实际浏览/跟船经过的 tile，不保证整个航区。
- 离线资料可能过期；它不替代官方海图、Notices to Mariners、测深仪或航行计划。
- 删除 App data/卸载 App 会删除私有离线地图；需要长期保存时应保留原始 MBTiles 文件。

## English

Yokuli never makes the alarm depend on map availability. Google Map, Satellite and the Google-hosted quiet Nautical base style are displayed only through the official online SDK and are never prefetched or cached by Yokuli. OpenSeaMap seamarks and LINZ Local depth tiles use separate 100 MB on-demand caches, refresh after seven days, and remain stale offline fallbacks after a refresh failure. OpenSeaMap/OpenStreetMap and LINZ attribution remain visible.
...
Clearing rebuildable caches removes recent OpenSeaMap/LINZ tiles and derived depth/grid caches, but never removes the user-imported MBTiles archive. The latter requires an explicit Remove action under Settings → Map data.
