...
> 文档对应 Room schema v15 / 当前 `BackdownCenterEstimator`、`AnchorCentreObservabilityPolicy` 与 `AnchorCentreRecalculator` 实现。它描述的是 Yokuli 自己的保守估算器，不声称复制 Garmin 或其他厂商未公开的专有算法。
...
只有至少 20 个点和 20 秒有效数据时才尝试拟合。圆拟合过程：
...
一个数学上漂亮的小圆并不等于观测到了真实锚链尺度。圆拟合还要经过独立的径向门：

$$
r_{fit} \ge \max(12\text{ m},0.35R_h,2.5m_{gps})
$$

$$
D_{track} \ge \max(12\text{ m},0.40R_h,3m_{gps})
$$

同时拟合半径不得大于 (R_h+m_{gps})，GPS 余量也不能占拟合半径或轨迹跨度的主要部分。任何一项失败都会得到明确原因（轨迹太小、拟合半径太小、与锚链几何冲突、GPS 不确定性占主导或无可用圆拟合），并保持 `LEARNING`。例如 40 m 水平锚链下，局部 5–10 m 小圈即使 RMS 很漂亮，也不能提前生成已确定锚点。
...
| 来源 | 当前权重 | 使用条件 | 含义 |
|---|---:|---|---|
| NMEA 真实船首向 | 1.00 | `NMEA_PHYSICAL`、独立序列、稳定窗口 | 船艏通常朝向锚的方向，可参与最快证据路径 |
| 手机方向 | 0.35 | `PHONE`、稳定且跨多个窗口 | 只作弱空间加权，不计为物理船首向，也不能缩短时间门槛 |
...
这里的“真实船首向”只指被标记为 `NMEA_PHYSICAL` 的 HDT/HDG/VHW 证据。手机磁力方向可以帮助可行区域排序，但不会让 `hasPhysicalEvidence` 成立，也不会把 15 分钟路径错误地加速成 5 分钟路径。
...
已处于 `RESOLVED` 的活动或历史 session 也可以对完整可信轨迹执行同一套离线分析，但结果始终是候选：

- 活动 session：只展示当前中心、候选中心、偏移方位、不确定度和径向证据；报警中心只有在用户明确确认后才改变，并保留原报警半径与轨迹。
- 已结束 session：分析不会篡改历史记录；用户只能把结果另存为带“不确定范围”的锚地参考。
- 报警处于 WARNING、ALARM 或 ACKNOWLEDGED 时禁止应用候选，避免在危险状态下用重算移动安全边界。
- “继续估算”会保存证据基线；之后至少增加 120 个样本，且覆盖增加 25°、轨迹跨度按锚链尺度增长或新增真实反转之一成立，才允许再次提出候选。
...
| 开始拟合的最少点数 | 20 |
| 开始拟合的最短有效时间 | 20 s |
| 径向最小轨迹跨度 | max(12 m, 40% (R_h), 3×GPS margin) |
| 径向最小拟合半径 | max(12 m, 35% (R_h), 2.5×GPS margin) |
...
- `domain/anchor/AnchorCentreObservability.kt`：锚链尺度径向可观测性、继续估算证据增长和候选显著偏移门。
- `domain/anchor/AnchorCentreRecalculator.kt`：已确定锚点的完整历史重算；只返回候选和失败原因。
