...
tag 推送后，本机工作结束。GitHub Actions 在线执行 Unit、Lint、Android 设备 story、签名、
APK/AAB 构建、签名验证和 SHA-256，全部通过后创建 GitHub Release 下载页。
...
After the immutable tag is pushed, GitHub Actions performs every quality gate, signing operation,
APK/AAB build, verification step, and GitHub Release upload online. Stop the local console with
`Control-C`. It never deletes, moves, or force-pushes a release tag.
