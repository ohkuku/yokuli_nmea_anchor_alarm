...
| Boat-network supplement destination | same input Socket, or explicit gateway TX host/port/UDP | `OutputSettingsRepository` | Data → Share → Boat-network supplement | `AnchorWatchNmeaPublisher` | Global config | Phone-hosted TCP server was incorrectly a transport mode | TCP server removed below UI; route saving and live lease are separate operations |
| Boat-network supplement lease | process-local `publicationEnabled` | `OutputSettingsRepository` plus runtime | Data → Share → Boat-network supplement Start/Stop | output runtime | Runtime only | config writes also mutated the live lease | explicit `requestStart` / `requestStop`; never restored automatically |
| Phone NMEA service configuration | listen port and optional locally-owned fields | `LocalNmeaServerSettingsRepository` | Data → Share → Phone NMEA service | `LocalNmeaServerRuntime` | Global config | shared OutputSettings, calibration gate and diagnostics with boat TX | independent DataStore and independent listener ownership |
| Phone NMEA service lease | process-local `serverRequested` | `LocalNmeaServerSettingsRepository` | Data → Share → Phone NMEA service Start/Stop | `LocalNmeaServerRuntime` / `NmeaSharingServer` | Runtime only | rapid duplicate Start could interleave Stop and new listener | serialized idempotent lifecycle; zero clients remains RUNNING |
...
- Data → Share first asks for direction. Boat-network supplement owns only Boat TX; Phone NMEA service owns only this phone's listener. Each has separate configuration, lease, status, errors and raw views.
