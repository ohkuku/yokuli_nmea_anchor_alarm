...
本测试执行两次：第一次选择 **TCP 客户端**，并为 RX 和 TX 使用不同端口；停止后选择 **TCP 服务器**，让一台海图仪或平板连接到页面显示的手机地址。两次都持续观察至少 10 分钟，并包含手机船首向数值完全不变化、Boat HDT/VHW 持续输入的时间段。保持输出运行，把 App 默认定位或一个已暂停锚警切到 NMEA，再切回系统 GNSS；不要开启全局 NMEA GPS proxy。

- 预期：RX 始终保持连接；TCP 客户端只连接所选 TX 端点；TCP 服务器属于同一个 NMEA Output 产品，而不是第二套“共享”开关；Phone Position/Heading/Motion/Pressure 与 App 推算风约 1 Hz，Phone HDT 即使数值不变也每秒完整发送且相邻间隔不超过 1.2 秒；同一秒到期的数据只形成一次合并 Socket 写入；任何线路都不得输出 Boat DBT/DPT/VHW、直接 Boat 风、Boat Position 或 Boat Heading；App/锚警切换到 NMEA 不会停止手机输出，也不会让 TX Position 改成 NMEA，切回系统 GNSS 同样无需重启输出；停止输出只关闭 TX 客户端或监听器，不能停止 RX；“实时输出”与“上次已停止会话”明确分开；没有 TCP Server 客户端时“实际写入”为 0。
