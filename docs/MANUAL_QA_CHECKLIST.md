...
本次 Boat Watch/NMEA/收藏锚地重构只执行了 Kotlin 编译检查；新增和修改的测试均按要求写入但未运行。文档中更早的测试次数属于旧 commit，不能证明当前工作树。没有运行模拟器、真机、完整测试套件、Lint 或真实 NMEA 网关测试。正式确认版本可用前，请在目标硬件上完成下列安全关键场景。遇到失败时，在确保安全的前提下保留当前 Session，导出 Support Bundle，保存截图或录屏，并记录实际发生时间。
...
### QA-P0-015 — 船载网络补数：端点测试与正式启动的区别

1. 在完成校准前，配置一个独立 TX 端点并运行端点测试。
2. 确认端点测试可以发送诊断数据，但 **开始向船载网络补数** 仍被阻止。
3. 完成 QA-P0-014，再只启动一次正式输出。

- 预期：正式启动要求船舶零位、已固定状态和船首向对齐全部完成；启动后只发送手机 GNSS、已校准手机船首向、手机 IMU、手机气压及带明确 `APP_DERIVED` 来源的 App 推算真风。船载输入的同名数据、水深和 STW 绝不能出现在输出；App 重启后绝不能自动恢复输出。
- 结果/反馈：____________________________________________________________________

### QA-P0-016 — 船载网络补数的 RX/TX 独立性与固定心跳

本测试执行两次：先使用权威的 **复用现有输入连接**；只有确实存在另一台网关/另一发送端口时，才明确选择高级 **独立 TCP/UDP 网关**。持续观察至少 10 分钟，并包含手机船首向数值完全不变化、Boat HDT/VHW 持续输入的时间段。保持输出运行，把 App 默认定位或一个已暂停锚警切到 NMEA，再切回系统 GNSS；不要开启全局 NMEA GPS proxy。

- 预期：RX 始终保持连接；默认补数不新建第二条 Boat Socket；高级客户端只连接用户明确填写且不与 RX 冲突的 TX 端点。Phone Position/Heading/Motion/Pressure 与 App 推算风约 1 Hz，Phone HDT 即使数值不变也每秒完整发送；任何线路都不得输出 Boat DBT/DPT/VHW、直接 Boat 风、Boat Position 或 Boat Heading。停止船网补数不停止 RX，也不停止“本机 NMEA 服务”；实时与上次已停止会话明确分开。
- 结果/接收端证据：______________________________________________________________

### QA-P0-016D — 本机 NMEA 服务是另一项独立产品

1. 保持船载网络补数关闭，在“数据 → 分享”打开 **在本机提供 NMEA 服务**。
2. 保存端口后只点一次 Start；在零客户端状态等待 10 分钟，再用另一台手机/dashboard 连接页面显示的地址。
3. 启动船载网络补数，让两者同时工作；分别停止其中一个，再观察另一个。
4. 快速重复点 Start（仅用于一次受控 QA），随后保持零客户端等待。

- 预期：一次 Start 后监听器持续运行，不出现约 1 ms 的 STARTING→STOPPED；零客户端不会自动停止。客户端只收到 Phone/App-owned 数据，不会收到 Boat 输入流。页面分别显示“已生成”和“已实际发送”原始数据，零客户端时实际发送为 0。两项产品配置、session、Socket、资源占用、Start/Stop 和诊断完全独立；停止一个不影响另一个或 NMEA RX。
- 结果/监听地址/客户端证据：_____________________________________________________

### QA-P0-016B — Hard Stop 后旧会话必须零字节

1. 在 TCP 客户端正在连接一个不会完成握手的 fake/隔离端点时立即点停止。
2. 分别正常运行船载网络补数和本机 NMEA 服务，在各自接收端持续抓包，点停止并记录 Stop UI 完成时刻。
3. 等待 60 秒；期间保持手机传感器和 NMEA 输入继续工作。
4. 再次 Start，确认新 session ID 与旧会话不同。

- 预期：阻塞连接被立即中断；各产品 Stop 完成后，其旧 session 都不再增加字节；本机监听 Socket 不接受新客户端；Live TX 清空，旧行只出现在明确标为“上次已停止会话”的历史区；再次启动不会回放旧队列，也不会改变另一个产品。
- 结果/最后旧会话字节时间戳：____________________________________________________

### QA-P0-016C — KC-2W + Raymarine 单次受控负载验证

1. KC-2W 与 Simrad IS42 正常工作时，开启“复用现有输入连接”的手机数据输出；不要反复连接/断开。
2. 在 App“实时输出状态”记录 session、发布代次、上次/最长写入耗时和卡死中止次数；同时记录 KC-2W DataList。
3. 开启 Raymarine MFD，连续观察至少 10 分钟；让手机船首向保持不变一段时间，再缓慢转动。
4. 若页面出现拥塞或卡死，只等待 App 现有有界重连流程完成；不要手动连续点重连。保存 Support Bundle 和两端时间戳。

- 预期：所有完整数据约 1 Hz；同一秒到期的语句形成一个连续 TCP payload；Heading 不因数值不变而消失；不能出现“全部消失后字段逐个回来”的追赶突发。正常写入应低于 500 ms；超过 500 ms 时页面明确显示拥塞且队列只留每流最新值；超过 3 秒时只中止当前传输代次一次，由原 RX 有界重连恢复，绝不新建并行的第二条 Boat Socket。
- 结果/KC-2W 与 App 时间戳：_____________________________________________________
...
- 预期：直接打开一个全屏完整详情，只显示一个可操作位置卡片，不重复显示“预览 + 详情”；历史数据中的通用名称“Main spot”应显示为 **主要锚泊位置**；Approach、Google Maps、编辑、受保护删除和二维码功能可用。
- 结果/反馈：____________________________________________________________________

### QA-P0-018B — 双二维码分享卡

从锚地库和 Watch 附近锚地卡各生成一次分享图，并通过聊天软件压缩后发送到另一台 Android/iPhone。

- 预期：图片显示 Boat Watch logo 与 `Developed aboard SV Yokuli`。扫描“打开地图位置”可用普通系统相机打开正确坐标；扫描“导入完整资料”必须由 Boat Watch 识别，并在写入前预览字段。两张码用途不会互换；照片、访问历史和设备标识不进入二维码。
- 结果/两台设备：________________________________________________________________
...
### QA-P0-026 — 全局预警声音与静音模式

把 Android 调成普通“静音”，但保持“闹钟音量”大于 0。分别用演示数据触发锚泊 WARNING（越过预警圈但尚未越过正式报警圈）和持续大风 WARNING。

- 预期：锁屏、App 在前台或后台时都能听到全局预警音并有振动；通知和 App 内均可点击“稍后提醒”。普通静音/振动模式不能压掉声音。闹钟音量为 0 时，布防前检查必须阻止用户误以为声音可用。
- 再测试：在 WARNING 阶段点“稍后提醒”，然后继续让轨迹或风速升级到正式 ALARM。
- 预期：正式 ALARM 立即打破旧的预警 Snooze 并重新响铃，不能等到预警 Snooze 到期。
- 勿扰模式：分别测试“允许闹钟”和“完全静音”。前者应响；后者由 Android 系统策略控制，App 设置页必须提供清晰入口，不能声称已经绕过系统策略。
- 结果/反馈：____________________________________________________________________
