...
- **向船载网络补充缺失数据**只发布手机或 App 自己拥有的值，例如手机 GNSS、校准后的手机艏向/运动、气压和明确由 App 计算的风；不会把刚从船上收到的仪表数据再转发一遍。默认复用现有全双工输入连接；独立网关 TCP/UDP 只作为用户明确选择的高级路线。
- **在本机提供 NMEA 服务**是完全独立的功能，供可信 LAN/VPN 上的另一台手机、平板或 dashboard 连接。它有自己的端口、启动/停止状态、客户端数量和生成/实际发送原始数据；启动或停止它都不会改变“向船载网络补数”。
...
两个 NMEA 产品在设置和运行时都彼此独立。“船载网络补数”只填入手机/App 自己拥有的数值，通常写入现有全双工连接；独立网关 TCP/UDP 只能作为明确开启的高级路线。“本机 NMEA 服务”则通过自己的监听端口向其他设备提供同类手机/App 数据，绝不转发船载输入流。手机位置输出与“使用船载 NMEA 位置作为 App GPS”仍在逻辑上硬互斥。
