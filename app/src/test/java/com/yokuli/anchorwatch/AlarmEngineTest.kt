...
class AlarmEngineTest{private val c=AnchorConfig(0.0,0.0,40.0,warningRadiusMeters=40.0,alarmRadiusMeters=50.0);private fun f(north:Double,t:Long)=NavigationFix(north/110540,0.0,receivedElapsedRealtime=t,sourceSentence="test",valid=true)
 ...
 @Test fun learningTracksGpsLossWithoutRadiusAlarms(){val e=AlarmEngine(gpsLossMillis=15000);assertEquals(AlarmState.LEARNING,e.learn(c,0).state);assertEquals(AlarmState.LEARNING,e.onFix(f(500.0,1000)).state);assertNull(e.onFix(f(500.0,2000)).type);assertEquals(AlarmType.GPS_DATA_LOST,e.tick(17000).type)}
}
