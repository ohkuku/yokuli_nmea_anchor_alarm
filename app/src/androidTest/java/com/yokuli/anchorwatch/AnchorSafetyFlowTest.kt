...
class AnchorSafetyFlowTest {
    ...
    @Test fun backdownStartsImmediatelyButKeepsCentreHiddenUntilConfidenceIsHigh() = runBlocking<Unit> {
        TestNmeaServer().use { server ->
            val profile=liveProfile(server,true);preferences.save(AppSettings(profile=profile,gpsDataSource=GpsDataSource.NMEA));connectAndAwaitFix(profile)
            ContextCompat.startForegroundService(context,Intent(context,AnchorForegroundService::class.java).setAction(AnchorForegroundService.ARM)
                .putExtra("lat",-36.8485).putExtra("lon",174.7633).putExtra("rode",40.0).putExtra("depth",8.0).putExtra("boatLength",10.0)
    ...
    @Test fun backdownCentreBecomesVisibleOnlyAfterAHighConfidenceDropClusterAndMovement() = runBlocking<Unit> {
        TestNmeaServer().use { server ->
            val baseLat=-36.8485;val baseLon=174.7633;server.setFix(baseLat,baseLon)
            val profile=liveProfile(server,true);preferences.save(AppSettings(profile=profile,gpsDataSource=GpsDataSource.NMEA));connectAndAwaitFix(profile)
            ContextCompat.startForegroundService(context,Intent(context,AnchorForegroundService::class.java).setAction(AnchorForegroundService.ARM)
                .putExtra("lat",baseLat).putExtra("lon",baseLon).putExtra("rode",40.0).putExtra("depth",8.0).putExtra("boatLength",10.0)
                .putExtra("warning",56.0).putExtra("alarm",70.0).putExtra("placement",AnchorPlacementMode.BACKDOWN.name).putExtra("rangeMode","ADVANCED").putExtra("safetyPreset","BALANCED"))
            ...
            val resolved=withTimeout(8_000){while(dao.active()?.centerStatus!=AnchorCenterStatus.RESOLVED.name)delay(100);dao.active()!!}
            assertEquals(sessionId,resolved.id)
            assertEquals(baseLat,resolved.anchorLatitude,4.0/110_540.0)
            assertTrue(dao.events(sessionId).first().any{it.type=="ANCHOR_CENTER_RESOLVED"})
        }
    }
    ...
    @Test fun alarmRangeCanChangeWithoutReplacingTheActiveSession() = runBlocking<Unit> {
        TestNmeaServer().use { server ->
            ...
            ContextCompat.startForegroundService(context,Intent(context,AnchorForegroundService::class.java).setAction(AnchorForegroundService.UPDATE_RADIUS)
                .putExtra("alarm",85.0).putExtra("rode",50.0).putExtra("depth",9.0).putExtra("boatLength",11.0).putExtra("rangeMode","ADVANCED").putExtra("safetyPreset","TOLERANT"))
            withTimeout(5_000){while(dao.active()?.alarmRadiusMeters!=85.0)delay(50)}
            assertEquals(sessionId,dao.active()?.id)
            assertEquals(85.0,dao.active()!!.alarmRadiusMeters,0.01)
