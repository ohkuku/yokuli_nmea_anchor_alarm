...
class Migration5To6Test {
    @Test fun migration19To20KeepsEveryLegacyRowAndCreatesPlaceSpotVisitLinks()=runBlocking{
        ...
        try{
            ...
            assertEquals(3,migrated.legacyVisitCount);assertEquals(0,migrated.visitCountCached)
            ...
            assertTrue(db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='anchorage_personal_assessments'").use{it.moveToFirst()})
            assertEquals(0L,db.query("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='anchorage_personal_ratings'").use{it.moveToFirst();it.getLong(0)})
            val assessmentColumns=db.query("PRAGMA table_info(anchorage_personal_assessments)").use{cursor->buildSet{while(cursor.moveToNext())add(cursor.getString(cursor.getColumnIndexOrThrow("name")))}}
            assertTrue(assessmentColumns.containsAll(setOf("wouldReturn","holding","comfort","shoreAccess","crowding","quietness","legacyOverallRating")))
            val assessment=database.anchorageMetadataDao().assessment(migrated.id)!!
            assertEquals("UNKNOWN",assessment.wouldReturn);assertEquals(4,assessment.legacyOverallRating)
