...
data class VesselMountCalibration(val version:Int=1,val bowAxis:DeviceBowAxis=DeviceBowAxis.TOP,val neutralQuaternion:SensorQuaternion=SensorQuaternion(1.0,0.0,0.0,0.0),val calibratedAt:Long=0)
...
class VesselMountCalibrationRepository @Inject constructor(@ApplicationContext private val context:Context){
    private object K{val version=intPreferencesKey("calibration_version");val axis=stringPreferencesKey("bow_axis");val w=doublePreferencesKey("neutral_w");val x=doublePreferencesKey("neutral_x");val y=doublePreferencesKey("neutral_y");val z=doublePreferencesKey("neutral_z");val at=longPreferencesKey("calibrated_at")}
    val calibration=context.mountStore.data.map{p->VesselMountCalibration(version=p[K.version]?:1,bowAxis=p[K.axis]?.let{runCatching{DeviceBowAxis.valueOf(it)}.getOrNull()}?:DeviceBowAxis.TOP,neutralQuaternion=SensorQuaternion(p[K.w]?:1.0,p[K.x]?:0.0,p[K.y]?:0.0,p[K.z]?:0.0).normalized(),calibratedAt=p[K.at]?:0)}
    suspend fun save(axis:DeviceBowAxis,q:SensorQuaternion){context.mountStore.edit{p->p[K.version]=(p[K.version]?:0)+1;p[K.axis]=axis.name;p[K.w]=q.w;p[K.x]=q.x;p[K.y]=q.y;p[K.z]=q.z;p[K.at]=System.currentTimeMillis()}}
    suspend fun restore(value:VesselMountCalibration){
        ...
        context.mountStore.edit{p->
            ...
            p[K.at]=value.calibratedAt.coerceAtLeast(0L)
        }
    }
}
...
class PhoneVesselAttitudeRepository @Inject constructor(@ApplicationContext context:Context,private val calibrationRepository:VesselMountCalibrationRepository):SensorEventListener{
    ...
    @Volatile private var calibration=VesselMountCalibration();private var currentQuaternion:SensorQuaternion?=null;private var gyroValues=DoubleArray(3);private var dynamicG=0.0;private var running=false;private var previousQuaternion:SensorQuaternion?=null
    init{scope.launch{calibrationRepository.calibration.collect{calibration=it}}}
    ...
    private fun publish(current:SensorQuaternion){
        ...
        if(calibration.calibratedAt<=0L){previousQuaternion=current;_sample.value=PhoneVesselAttitudeSample(receivedElapsedRealtime=now);return}
        val corrected=(calibration.neutralQuaternion.inverse()*current*axisCorrection(calibration.bowAxis)).normalized();val sinRoll=2*(corrected.w*corrected.x+corrected.y*corrected.z);val cosRoll=1-2*(corrected.x*corrected.x+corrected.y*corrected.y);val roll=Math.toDegrees(atan2(sinRoll,cosRoll));val sinPitch=2*(corrected.w*corrected.y-corrected.z*corrected.x);val pitch=Math.toDegrees(if(abs(sinPitch)>=1)if(sinPitch>=0)Math.PI/2 else -Math.PI/2 else asin(sinPitch));val rates=mapRates(gyroValues,calibration.bowAxis);val previous=previousQuaternion;previousQuaternion=current;val jump=previous?.let{old->2*Math.toDegrees(acos(abs(old.w*current.w+old.x*current.x+old.y*current.y+old.z*current.z).coerceIn(0.0,1.0)))>35.0}?:false;val angular=sqrt(gyroValues.sumOf{it*it});_sample.value=PhoneVesselAttitudeSample(VesselAttitude(roll,pitch,Math.toDegrees(rates[0]),Math.toDegrees(rates[1]),Math.toDegrees(rates[2])),dynamicG,jump||angular>3.5,now)
    }
