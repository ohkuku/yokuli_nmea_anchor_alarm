...
class AnchorForegroundService:Service(){
 ...
 private suspend fun restoreState(){
  val settings=preferences.settings.first();currentGpsSource=settings.gpsDataSource;alarmSnoozeMinutes=settings.alarmSnoozeMinutes;selectedAlarmSound=settings.alarmSound;customAlarmSoundUri=settings.customAlarmSoundUri;appLanguage=settings.appLanguage;channels();session=dao.active();if(session==null)alarmUi.clear();session?.let{active->
   ...
   if(active.centerStatus==AnchorCenterStatus.LEARNING.name){backdownSamples.addAll(points.map{BackdownCenterEstimator.Sample(latitude=it.latitude,longitude=it.longitude,timestamp=it.timestamp,hdop=it.hdop,headingTrueDegrees=if(it.headingMeasured)it.heading else null,cogTrueDegrees=it.cog,sogKnots=it.sog,windDirectionTrueDegrees=it.windDirectionTrue,windSpeedKnots=it.windSpeedKnots,apparentWindAngleDegrees=it.apparentWindAngle,trueWindAngleDegrees=it.trueWindAngle,trueWindSpeedKnots=it.trueWindSpeedKnots,apparentWindSpeedKnots=it.apparentWindSpeedKnots,headingSampleSequence=it.headingSampleSequence,windSampleSequence=it.windSampleSequence)});lastSnapshot=engine.learn(active.learningConfig(),SystemClock.elapsedRealtime())}
 ...
 private suspend fun arm(request:ArmRequest){
  ...
  val c=if(settings.gpsDataSource==GpsDataSource.DEMO)request.config.copy(latitude=latestFix!!.latitude,longitude=latestFix.longitude)else request.config
 ...
 private suspend fun switchWatchToSystem(disconnectNmea:Boolean,disableDemoMode:Boolean=false){
  ...
  if(settings.demoMode&&(active!=null||!disableDemoMode)){notifySeparate("GPS source not changed","Demo mode locks this App to Demo GPS. Lift anchor and disable Demo mode first.",true);refreshNotification();return}
 ...
 private suspend fun switchWatchToNmea(){
  ...
  if(availability!=NmeaSourceAvailability.AVAILABLE||!navigation.claimBackgroundConnectionIfConnected()){
   logEvent("WATCH_GPS_SOURCE_CHANGE_REJECTED","NMEA_${availability.name}")
   notifySeparate("GPS source not changed","Connect the NMEA source and wait for a fresh valid position before selecting NMEA GPS.",true);refreshNotification();releaseIfIdle();return
  }
 ...
 private suspend fun handleFix(fix:NavigationFix,source:GpsDataSource){
  ...
  if(activeSession!=null&&!activeSession.paused){
   ...
   if(fix.valid&&System.currentTimeMillis()-lastTrack>=1000){
    lastTrack=System.currentTimeMillis();dao.insertPoint(TrackPointEntity(sessionId=activeSession.id,timestamp=lastTrack,latitude=fix.latitude,longitude=fix.longitude,distanceFromAnchor=snapshot.distanceMeters?:0.0,sog=fix.sogKnots,cog=fix.cogTrueDegrees,heading=fix.headingTrueDegrees,hdop=fix.hdop,windDirectionTrue=fix.windDirectionTrueDegrees,windSpeedKnots=fix.windSpeedKnots,apparentWindAngle=fix.apparentWindAngleDegrees,trueWindAngle=fix.trueWindAngleDegrees,trueWindSpeedKnots=fix.trueWindSpeedKnots,apparentWindSpeedKnots=fix.apparentWindSpeedKnots,headingMeasured=fix.headingTrueDegrees!=null,headingSampleSequence=fix.headingSampleSequence,windSampleSequence=fix.windSampleSequence))
    if(activeSession.centerStatus==AnchorCenterStatus.LEARNING.name){
     backdownSamples+=BackdownCenterEstimator.Sample(latitude=fix.latitude,longitude=fix.longitude,timestamp=lastTrack,hdop=fix.hdop,headingTrueDegrees=fix.headingTrueDegrees,cogTrueDegrees=fix.cogTrueDegrees,sogKnots=fix.sogKnots,windDirectionTrueDegrees=fix.windDirectionTrueDegrees,windSpeedKnots=fix.windSpeedKnots,apparentWindAngleDegrees=fix.apparentWindAngleDegrees,trueWindAngleDegrees=fix.trueWindAngleDegrees,trueWindSpeedKnots=fix.trueWindSpeedKnots,apparentWindSpeedKnots=fix.apparentWindSpeedKnots,headingSampleSequence=fix.headingSampleSequence,windSampleSequence=fix.windSampleSequence)
     val estimate=backdownEstimator.provisionalEstimate(backdownSamples,activeSession.expectedSwingRadiusMeters)
     if(estimate!=null){
 ...
 private suspend fun refineEstimatedCenter(fix:NavigationFix):Boolean{
  val current=session?:return false;val now=SystemClock.elapsedRealtime();if(current.placementMode!=AnchorPlacementMode.BACKDOWN.name||current.centerStatus!=AnchorCenterStatus.RESOLVED.name||centerPoints.size<300||now-lastCenterUpdate<30_000)return false
 ...
 private fun sound(){
  ...
  val systemAlarm=android.provider.Settings.System.DEFAULT_ALARM_ALERT_URI
  val selected=when(selectedAlarmSound){AlarmSound.SYSTEM_ALARM->systemAlarm;AlarmSound.SYSTEM_RINGTONE->android.provider.Settings.System.DEFAULT_RINGTONE_URI;AlarmSound.SYSTEM_NOTIFICATION->android.provider.Settings.System.DEFAULT_NOTIFICATION_URI;AlarmSound.CUSTOM->customAlarmSoundUri?.let{runCatching{android.net.Uri.parse(it)}.getOrNull()}}
  if(!playAlarm(selected)&&selected!=systemAlarm)playAlarm(systemAlarm)
 ...
 private suspend fun pauseWatch(){
  ...
  engine.stop();lastSnapshot=AlarmSnapshot(AlarmState.STOPPED);alarmUi.clear();lastReportedAlarm=null;nmeaLossAnnounced=false;silence();getSystemService(NotificationManager::class.java).cancel(EVENT);refreshNotification();releaseIfIdle()
 }
