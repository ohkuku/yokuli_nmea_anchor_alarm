...
internal fun WatchPage(state: MainUiState, vm: MainViewModel) {
    ...
    LaunchedEffect(sonarGrid,state.sonarGridVersion){val version=state.sonarGridVersion;if(sonarTileProvider.updateGrid(sonarGrid,state.sonarGridChangedCells.takeIf{it.isNotEmpty()})>0)sonarTileOverlayState.clearTileCache();vm.consumeSonarGridChanges(version)}
        ...
        Box(Modifier.weight(1f).background(MaterialTheme.colorScheme.surfaceVariant)) {
            if (BuildConfig.MAPS_CONFIGURED) {
                    GoogleMap(Modifier.fillMaxSize(), cameraPositionState = camera, properties = MapProperties(mapType = if(state.settings.mapType==2)MapType.SATELLITE else MapType.NORMAL), uiSettings = MapUiSettings(compassEnabled = false, indoorLevelPickerEnabled = false, mapToolbarEnabled = false, myLocationButtonEnabled = false, zoomControlsEnabled = false,scrollGesturesEnabled=!mapLocked,zoomGesturesEnabled=!mapLocked,rotateGesturesEnabled=!mapLocked,tiltGesturesEnabled=!mapLocked), onMapLoaded = { mapLoaded = true },onMapClick={point->
                        ...
                        if(state.settings.sonarLayerEnabled&&sonarGrid.cells.isNotEmpty())TileOverlay(tileProvider=sonarTileProvider,state=sonarTileOverlayState,fadeIn=false,transparency=(1.0-state.settings.sonarLayerOpacity.coerceIn(.20,1.0)).toFloat(),visible=true,zIndex=MapOverlayZ.SONAR)
