...
@Composable private fun PhoneVesselSensorsCard(state:MainUiState,vm:MainViewModel){
 ...
 Card{Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
  ...
  Text(tr("Phone capabilities are independent. GNSS and pressure need no vessel mount. Vessel heading uses one durable phone-to-bow alignment. Sailing attitude is configured only inside Trip Watch.","手机的各项能力彼此独立：GNSS 和气压不需要固定安装；船首向只使用一次性的手机—船艏对齐；航行姿态仅在 Trip Watch 内设置。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
  ...
  HorizontalDivider()
  PhoneSensorStep(1,alignmentReady,tr("Align phone direction to the vessel bow","将手机方向与船艏对齐"),tr("Save the angular offset once. Translating the phone does not affect it. If you rotate the phone independently, vessel heading follows that rotation until you place it back; the App cannot distinguish handset rotation from vessel yaw without an external reference.","偏角只需保存一次。平移手机不会影响它；若单独旋转手机，船首向会随手机转动，直到你把手机放回原方向。没有外部参考时，App 无法区分手机旋转与船体转艏。"))
  Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(8.dp),verticalAlignment=Alignment.CenterVertically){
   ...
   Button({alignmentValue?.let(vm::setPhoneHeadingAlignment)},enabled=alignmentDirty,modifier=Modifier.testTag("phone_sensor_confirm_heading")){Text(tr("Confirm","确认"))}
  }
  HorizontalDivider()
  Surface(color=if(productionReady)MaterialTheme.colorScheme.primaryContainer else MaterialTheme.colorScheme.errorContainer,shape=MaterialTheme.shapes.medium,modifier=Modifier.testTag("phone_sensor_readiness")){Column(Modifier.fillMaxWidth().padding(12.dp),verticalArrangement=Arrangement.spacedBy(4.dp)){Text(if(productionReady)tr("HEADING ALIGNMENT READY","船首向对齐已就绪")else tr("HEADING ALIGNMENT REQUIRED","需要船首向对齐"),fontWeight=FontWeight.Bold);Text(if(productionReady)tr("Phone/App sharing may publish aligned vessel heading. GNSS and pressure remain independent; Trip attitude has its own in-trip confirmation.","手机 / App 分享可发布对齐后的船首向。GNSS 与气压保持独立；航程姿态在 Trip Watch 内单独确认。")else tr("Confirm the heading offset once before formal Phone/App sharing. Attitude placement is not a prerequisite here.","正式分享手机 / App 数据前只需确认一次艏向偏差；这里不要求设置姿态安装。"),style=MaterialTheme.typography.bodySmall)}}
  ...
  val alignedHeading=state.phoneHeading.liveTrueHeadingDegrees?.let{((it+state.vesselMountCalibration.headingAlignmentOffsetDegrees)%360.0+360.0)%360.0}
  Text("${tr("Device direction","手机方向")} ${state.phoneHeading.liveTrueHeadingDegrees?.let{"%03.0f°T".format(it)}?:"—"} · ${tr("Aligned vessel heading","对齐后船首向")} ${alignedHeading?.takeIf{alignmentReady}?.let{"%03.0f°T".format(it)}?:"—"}",style=MaterialTheme.typography.bodyMedium)
...
@Composable private fun NmeaProductsOverview(state:MainUiState,onBoat:()->Unit,onLocal:()->Unit,onStopAll:()->Unit){
 ...
 LazyColumn(Modifier.fillMaxSize().padding(16.dp).testTag("nmea_products_overview"),verticalArrangement=Arrangement.spacedBy(12.dp)){
  ...
  item{ElevatedCard(onClick=onBoat,modifier=Modifier.fillMaxWidth().testTag("nmea_product_boat_network")){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   ...
   Text(tr("Continuously writes every valid Phone/App-owned measurement into the NMEA network. It never repeats Boat input; receiving instruments choose their preferred source. The normal route reuses the connected full-duplex TCP socket.","把每个有效的手机 / App 自有测量值持续写入 NMEA 船网；绝不重复船载输入，由接收端仪表选择来源。通常复用已经连接的全双工 TCP Socket。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
...
@Composable private fun PhoneAppBoatOutputPage(state:MainUiState,vm:MainViewModel,back:()->Unit){
 ...
 LazyColumn(Modifier.fillMaxSize().testTag("nmea_output_list").padding(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
  ...
  item{Card{Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   ...
   Surface(color=if(phoneFrameReady)MaterialTheme.colorScheme.secondaryContainer else MaterialTheme.colorScheme.surfaceVariant,shape=MaterialTheme.shapes.small){Text(if(phoneFrameReady)tr("The one-time phone heading alignment is confirmed. GNSS and pressure are independent; Motion appears only during a confirmed Trip attitude segment.","一次性手机船首向对齐已确认。GNSS 与气压独立工作；只有 Trip Watch 中确认了有效姿态段时才会发送运动数据。")else tr("Sharing is locked until Settings → Phone vessel sensors confirms the phone-to-bow heading offset. A fixed attitude mount is not a global prerequisite.","在“设置 → 手机船舶传感器”确认手机到船艏的方位偏差前，分享保持锁定；固定姿态安装不再是全局前置条件。"),Modifier.fillMaxWidth().padding(10.dp),style=MaterialTheme.typography.bodySmall)}
  ...
  item{Card(colors=CardDefaults.cardColors(containerColor=if(outputActive)MaterialTheme.colorScheme.primaryContainer else MaterialTheme.colorScheme.surfaceVariant)){Column(Modifier.fillMaxWidth().padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   Row(Modifier.fillMaxWidth(),verticalAlignment=Alignment.CenterVertically){Icon(if(outputActive)Icons.Default.Sensors else Icons.Default.StopCircle,null,tint=if(outputActive)MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.onSurfaceVariant);Spacer(Modifier.width(10.dp));Column(Modifier.weight(1f)){Text(if(outputActive)tr("Phone/App data is being sent","正在发送手机 / App 数据")else tr("Ready to send Phone/App data","可以发送手机 / App 数据"),fontWeight=FontWeight.SemiBold);Text(if(outputActive&&state.outputSettings.transportMode==NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION)tr("TX is sharing the input TCP socket. Stop sending before stopping RX.","TX 正与输入共用 TCP Socket；停止 RX 前请先停止发送。")else if(outputActive)tr("Independent boat TX is running separately from NMEA input.","独立船网 TX 与 NMEA 输入分开运行。")else tr("Configure the boat route above. Saving never starts a socket by itself.","请先配置上方船网线路；仅保存配置绝不会自行启动 Socket。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)}}
   ...
   if(!headingAlignmentReady)Text(tr("Heading alignment required: confirm the phone-to-bow offset in Settings → Phone vessel sensors. Trip attitude placement is optional and independent.","需要船首向对齐：请在“设置 → 手机船舶传感器”确认手机到船艏的偏差。航程姿态放置是可选且独立的。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.error)
  ...
  item{Card(Modifier.testTag("nmea_output_live_status")){Column(Modifier.fillMaxWidth().padding(14.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   ...
   val configuredEndpoint=NmeaOutputEndpointPolicy.resolved(state.outputSettings,state.settings.profile)
   val visibleMode=if(outputActive||outputRequested)state.phonePositionOutputStatus.mode else state.outputSettings.transportMode
...
@Composable private fun LocalNmeaServicePage(state:MainUiState,vm:MainViewModel,back:()->Unit){
 ...
 LazyColumn(Modifier.fillMaxSize().padding(16.dp).testTag("local_nmea_server_page"),verticalArrangement=Arrangement.spacedBy(12.dp)){
  ...
  item{Card(colors=CardDefaults.cardColors(containerColor=if(requested)MaterialTheme.colorScheme.primaryContainer else MaterialTheme.colorScheme.surfaceVariant)){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   ...
   if(!frameReady)Text(tr("Confirm the one-time heading alignment in Settings → Phone vessel sensors before starting. Trip attitude movement never stops this listener; only the optional Motion stream pauses.","启动前请在“设置 → 手机船舶传感器”确认一次性船首向对齐。航程姿态移动绝不会停止监听；只会暂停可选的运动数据流。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.error)
