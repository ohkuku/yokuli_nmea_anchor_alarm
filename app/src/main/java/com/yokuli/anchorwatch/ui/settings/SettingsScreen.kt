...
@Composable internal fun DataOutputSettingsPage(state:MainUiState,vm:MainViewModel){
 ...
 val vesselFrameReady=state.vesselMountCalibration.calibratedAt>0L&&state.phoneVesselMountState==PhoneVesselMountState.VESSEL_MOUNTED
 ...
 LazyColumn(Modifier.fillMaxSize().padding(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
  ...
  item{Card(colors=CardDefaults.cardColors(containerColor=if(outputActive)MaterialTheme.colorScheme.primaryContainer else MaterialTheme.colorScheme.surfaceVariant)){Column(Modifier.fillMaxWidth().padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   ...
   if(state.vesselMountCalibration.calibratedAt<=0L)Text(tr("Phone vessel-sensor calibration is required before Test or Start can open an output connection.","测试或启动输出连接前，必须先完成手机船舶传感器校准。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.error)
   else if(!vesselFrameReady&&(state.outputSettings.effectiveHeadingPolicy!=PublicationPolicy.OFF||state.outputSettings.effectiveMotionPolicy!=PublicationPolicy.OFF))Text(tr("Heading or motion is selected: secure the calibrated phone to the vessel before Start output can succeed.","已选择船首向或船体运动：启动输出前必须将已校准手机牢固固定在船体。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.error)
  ...
  item{Card(Modifier.testTag("nmea_output_route")){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
   ...
   HorizontalDivider()
   PublicationPolicyRow(tr("Phone vessel heading (HDG/HDT)","手机船体船首向 (HDG/HDT)"),state.outputSettings.effectiveHeadingPolicy,(outputReady&&vesselFrameReady)||state.outputSettings.effectiveHeadingPolicy!=PublicationPolicy.OFF){setPolicy("heading",it)}
   ...
   PublicationPolicyRow(tr("Phone vessel motion (ROT/XDR)","手机船体运动 (ROT/XDR)"),state.outputSettings.effectiveMotionPolicy,state.outputSettings.effectiveMotionPolicy!=PublicationPolicy.OFF||(outputReady&&state.phoneSensorCapabilities.attitudeAvailable&&vesselFrameReady)){setPolicy("motion",it)}
   ...
   OutlinedButton({testResult=null;vm.testNmeaDeviceOutput{success->testResult=if(success)testWrittenMessage else testFailedMessage}},Modifier.fillMaxWidth(),enabled=outputReady&&!outputActive&&state.vesselMountCalibration.calibratedAt>0L){Text(tr("Test NMEA output","测试 NMEA 输出"))}
   ...
   if(showPublisherDiagnostics){
    state.phonePositionOutputStatus.streams.filterKeys{it in setOf("POSITION","HEADING","MOTION","PRESSURE","DERIVED_WIND","STATUS")}.forEach{(stream,status)->
     ...
     TextButton({confirmHeadingDiagnostic=true},Modifier.fillMaxWidth(),enabled=outputReady&&!outputActive&&state.vesselMountCalibration.calibratedAt>0L){Text(tr("Developer diagnostic · 5 × IIHDG 123.4°","开发诊断 · 发送 5 条 IIHDG 123.4°"))}
   }
  ...
  item{Card(Modifier.testTag("nmea_raw_output")){Column(Modifier.fillMaxWidth().padding(14.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   ...
   SelectionContainer{Surface(Modifier.fillMaxWidth(),color=Color.Black,shape=MaterialTheme.shapes.small){Column(Modifier.fillMaxWidth().padding(10.dp),verticalArrangement=Arrangement.spacedBy(3.dp)){
    if(state.phonePositionOutputStatus.recentGenerated.isEmpty())Text(tr("No output sentence has been generated in this App run.","本次 App 运行期间尚未生成任何输出语句。"),color=Color.Gray,fontFamily=FontFamily.Monospace,style=MaterialTheme.typography.bodySmall)
    else state.phonePositionOutputStatus.recentGenerated.takeLast(40).asReversed().forEach{line->Text(line,color=Color(0xFFB9F6CA),fontFamily=FontFamily.Monospace,style=MaterialTheme.typography.labelSmall)}
   }}}
   ...
   SelectionContainer{Surface(Modifier.fillMaxWidth(),color=Color.Black,shape=MaterialTheme.shapes.small){Column(Modifier.fillMaxWidth().padding(10.dp),verticalArrangement=Arrangement.spacedBy(3.dp)){
    if(state.phonePositionOutputStatus.recentTx.isEmpty())Text(tr("No successful socket write yet.","尚无成功的 Socket 写入。"),color=Color.Gray,fontFamily=FontFamily.Monospace,style=MaterialTheme.typography.bodySmall)
    else state.phonePositionOutputStatus.recentTx.takeLast(40).asReversed().forEach{line->Text(line,color=Color(0xFF80CBC4),fontFamily=FontFamily.Monospace,style=MaterialTheme.typography.labelSmall)}
   }}}
  }}}
