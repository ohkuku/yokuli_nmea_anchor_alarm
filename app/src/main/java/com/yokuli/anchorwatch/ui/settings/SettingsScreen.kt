...
@Composable private fun BoatNetworkSupplementPage(state:MainUiState,vm:MainViewModel,back:()->Unit){
 ...
 LazyColumn(Modifier.fillMaxSize().testTag("nmea_output_list").padding(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
  ...
  item{Card{Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   ...
   Text(tr("Published: Phone GNSS, calibrated Phone heading, Phone IMU, Phone barometer, and true wind only when Boat Watch computed it. Never published: Boat depth, STW, direct Boat wind, Boat position or Boat heading.","可发布：手机 GNSS、已校准手机船首向、手机 IMU、手机气压，以及仅由 Boat Watch 自行推算的真风。绝不发布：船载水深、对水航速、船载直接风、船载船位或船载船首向。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
