...
@Composable internal fun SettingsPageContent(state:MainUiState,vm:MainViewModel){
 var destination by rememberSaveable{mutableStateOf(SettingsDestination.ROOT)};var feedbackReturn by rememberSaveable{mutableStateOf(SettingsDestination.ROOT)};var languageDialog by remember{mutableStateOf(false)};var pendingSupportUrl by remember{mutableStateOf<String?>(null)};val chinese=state.settings.appLanguage==AppLanguage.SIMPLIFIED_CHINESE||(state.settings.appLanguage==AppLanguage.SYSTEM&&state.settings.appLanguage.usesChinese());val context=androidx.compose.ui.platform.LocalContext.current;val externalLauncher=remember(context){ExternalLinkLauncher(context)};BackHandler(destination!=SettingsDestination.ROOT){destination=if(destination==SettingsDestination.FEEDBACK)feedbackReturn else SettingsDestination.ROOT}
 when(destination){
