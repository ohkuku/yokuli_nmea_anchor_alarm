...
private enum class SettingsDestination{ROOT,ALARM,VESSEL,DEPTH_SOUNDER,POSITIONING,MAP_DEPTH,DATA_OUTPUT,BACKGROUND,DATA_BACKUP,STORAGE_SUPPORT,DEVELOPER,ABOUT,FEEDBACK}
...
@Composable private fun SettingsRoot(state:MainUiState,open:(SettingsDestination)->Unit,language:()->Unit,support:(String)->Unit){
 LazyColumn(Modifier.fillMaxSize().padding(horizontal=18.dp).testTag("settings_list"),contentPadding=PaddingValues(vertical=18.dp)){
  ...
  item{SettingsSection(tr("DEVICE & DATA","设备与数据"));SettingsRow(Icons.Default.Share,tr("Data output","数据输出"),tr("NMEA sharing server · phone GPS to boat","NMEA 共享服务器 · 手机 GPS 写回船网"),"settings_data_output"){open(SettingsDestination.DATA_OUTPUT)};SettingsRow(Icons.Default.BatterySaver,tr("Background reliability","后台可靠性"),tr("Permissions, power and Wi-Fi","权限、电源与 Wi-Fi"),"settings_background"){open(SettingsDestination.BACKGROUND)};SettingsRow(Icons.Default.Backup,tr("Data & backup","数据与备份"),tr("Export or replace from an Anchor Watch backup","导出或从 Anchor Watch 备份替换恢复"),"settings_data_backup"){open(SettingsDestination.DATA_BACKUP)};SettingsRow(Icons.Default.Storage,tr("Storage & support","存储与支持"),tr("Health, incident log and diagnostics","健康、事件日志与诊断包"),"settings_storage_support"){open(SettingsDestination.STORAGE_SUPPORT)};SettingsRow(Icons.Default.Language,tr("Language","语言"),state.settings.appLanguage.nativeName,"settings_language",language)}
...
@Composable private fun SettingsSubPage(destination:SettingsDestination,state:MainUiState,vm:MainViewModel,back:()->Unit){Column(Modifier.fillMaxSize()){Row(Modifier.fillMaxWidth().padding(horizontal=8.dp,vertical=6.dp),verticalAlignment=Alignment.CenterVertically){IconButton(back){Icon(Icons.AutoMirrored.Filled.ArrowBack,tr("Back","返回"))};Text(when(destination){SettingsDestination.ALARM->tr("Alarm & notifications","报警与通知");SettingsDestination.VESSEL->tr("Vessel & sensors","船舶与传感器");SettingsDestination.DEPTH_SOUNDER->tr("Depth sounder","测深仪");SettingsDestination.POSITIONING->tr("Positioning","定位");SettingsDestination.MAP_DEPTH->tr("Map & depth","地图与水深");SettingsDestination.DATA_OUTPUT->tr("Data output","数据输出");SettingsDestination.BACKGROUND->tr("Background reliability","后台可靠性");SettingsDestination.DATA_BACKUP->tr("Data & backup","数据与备份");SettingsDestination.STORAGE_SUPPORT->tr("Storage & support","存储与支持");SettingsDestination.DEVELOPER->tr("Developer","开发者");else->""},style=MaterialTheme.typography.titleLarge,fontWeight=FontWeight.SemiBold)};LazyColumn(Modifier.fillMaxSize().padding(horizontal=16.dp),contentPadding=PaddingValues(bottom=24.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){item{when(destination){SettingsDestination.ALARM->AlarmSettingsPage(state,vm);SettingsDestination.VESSEL->Column(verticalArrangement=Arrangement.spacedBy(12.dp)){VesselProfileCard(state,vm);VesselDataSourcesCard(state,vm);PhoneVesselSensorsCard(state,vm)};SettingsDestination.DEPTH_SOUNDER->DepthSounderPage(state,vm);SettingsDestination.POSITIONING->GpsDataSourceCard(state,vm);SettingsDestination.MAP_DEPTH->MapDepthSettingsPage(state,vm);SettingsDestination.DATA_OUTPUT->DataOutputSettingsPage(state,vm);SettingsDestination.BACKGROUND->BackgroundReliabilityCard(state,vm);SettingsDestination.DATA_BACKUP->DataBackupPage(state,vm);SettingsDestination.STORAGE_SUPPORT->StorageSupportPage(state,vm);SettingsDestination.DEVELOPER->DeveloperSettingsCard(state,vm);else->{}}}}}}
...
@Composable private fun VesselDataSourcesCard(state:MainUiState,vm:MainViewModel){
 @Composable fun sourceLabel(value:com.yokuli.anchorwatch.domain.vessel.VesselObservation<*>)="${when(value.source){com.yokuli.anchorwatch.domain.vessel.VesselDataSource.BOAT_NMEA->tr("Boat NMEA","船载 NMEA");com.yokuli.anchorwatch.domain.vessel.VesselDataSource.PHONE_GNSS,com.yokuli.anchorwatch.domain.vessel.VesselDataSource.PHONE_MAGNETOMETER->tr("Phone","手机");else->tr("Unavailable","不可用")}} · ${when(value.freshness){VesselDataFreshness.FRESH->tr("Fresh","新鲜");VesselDataFreshness.HELD->tr("Held","保留值");VesselDataFreshness.STALE->tr("Stale","已过期");VesselDataFreshness.UNAVAILABLE->tr("Unavailable","不可用")}}"
 @Composable fun selector(title:String,value:VesselSourcePreference,summary:String,change:(VesselSourcePreference)->Unit){Column(verticalArrangement=Arrangement.spacedBy(7.dp)){Text(title,fontWeight=FontWeight.SemiBold);Text(summary,style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant);SingleChoiceSegmentedButtonRow(Modifier.fillMaxWidth()){VesselSourcePreference.entries.forEachIndexed{index,item->SegmentedButton(value==item,{change(item)},shape=SegmentedButtonDefaults.itemShape(index,VesselSourcePreference.entries.size)){Text(when(item){VesselSourcePreference.AUTO->"AUTO";VesselSourcePreference.BOAT->tr("Boat","船载");VesselSourcePreference.PHONE->tr("Phone","手机")})}}}}}
 Card{Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(14.dp)){
  Text(tr("Display data sources","显示数据源"),style=MaterialTheme.typography.titleMedium)
  Text(tr("AUTO fallback applies only to instruments and Trip Watch. Anchor safety remains locked to the source chosen when armed.","AUTO 回退只用于仪表和航行监控；锚警始终锁定布防时选择的数据源。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
  selector(tr("Position","位置"),state.vesselSettings.positionPreference,sourceLabel(state.vesselData.position)){vm.updateVesselDataSettings(state.vesselSettings.copy(positionPreference=it))}
  HorizontalDivider()
  selector(tr("Heading","船首向"),state.vesselSettings.headingPreference,sourceLabel(state.vesselData.headingTrueDegrees)){vm.updateVesselDataSettings(state.vesselSettings.copy(headingPreference=it))}
  Text(tr("Depth and wind always come from Boat NMEA. Phone attitude and pressure use phone sensors when available.","水深和风始终来自船载 NMEA；手机姿态和气压在传感器可用时来自手机。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
 }}
}
...
@Composable private fun PhoneVesselSensorsCard(state:MainUiState,vm:MainViewModel){
 ...
 Card{Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
  ...
  SingleChoiceSegmentedButtonRow(Modifier.fillMaxWidth()){DeviceBowAxis.entries.forEachIndexed{index,value->SegmentedButton(axis==value,{axis=value},shape=SegmentedButtonDefaults.itemShape(index,DeviceBowAxis.entries.size)){Text(value.name.lowercase().replaceFirstChar{it.uppercase()})}}}
...
@Composable private fun DataOutputSettingsPage(state:MainUiState,vm:MainViewModel){
 ...
 val canEnable=!activeNmea&&!nmeaSelected&&writable
 Column(verticalArrangement=Arrangement.spacedBy(12.dp)){
  Card{Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
  ...
  Card{Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
   Text(tr("NMEA Device Output","NMEA 设备输出"),style=MaterialTheme.typography.titleMedium)
   Text(tr("Writes phone/derived measurements to the connected boat TCP endpoint. Boat input is never echoed back.","把手机或派生数据写入已连接的船载 TCP 端点；船载输入绝不会原样回写。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
   SettingSwitch(tr("Phone Position","手机位置"),if(canEnable||state.outputSettings.phonePositionEnabled)tr("RMC · GGA · VTG · ZDA","RMC · GGA · VTG · ZDA")else if(nmeaSelected||activeNmea)tr("Unavailable while NMEA Position is the App GPS source.","当 NMEA 位置是本应用 GPS 来源时不可用。")else tr("Connect a TCP NMEA endpoint first.","请先连接 TCP NMEA 端点。"),state.outputSettings.phonePositionEnabled,canEnable||state.outputSettings.phonePositionEnabled,vm::setPhonePositionOutput)
   HorizontalDivider()
   SettingSwitch(tr("Phone heading","手机船首向"),tr("HDT · live compass heading","HDT · 实时指南针方向"),state.outputSettings.phoneHeadingEnabled,writable||state.outputSettings.phoneHeadingEnabled,vm::setPhoneHeadingOutput)
   SettingSwitch(tr("Phone vessel motion","手机船体运动"),if(state.vesselMountCalibration.calibratedAt>0)tr("ROT · XDR heel and pitch","ROT · XDR 横倾与纵倾")else tr("Set vessel zero in Vessel & sensors first.","请先在“船舶与传感器”设置船体零点。"),state.outputSettings.phoneMotionEnabled,state.outputSettings.phoneMotionEnabled||(writable&&state.phoneSensorCapabilities.attitudeAvailable&&state.vesselMountCalibration.calibratedAt>0),vm::setPhoneMotionOutput)
   SettingSwitch(tr("Phone pressure","手机气压"),tr("XDR barometric pressure · 1 Hz","XDR 气压 · 1 Hz"),state.outputSettings.phonePressureEnabled,state.outputSettings.phonePressureEnabled||(writable&&state.phoneSensorCapabilities.pressureAvailable),vm::setPhonePressureOutput)
   SettingSwitch(tr("Yokuli sensor status","Yokuli 传感器状态"),tr("Optional PYOK diagnostic sentence","可选 PYOK 诊断语句"),state.outputSettings.proprietaryStatusEnabled,writable||state.outputSettings.proprietaryStatusEnabled,vm::setPhoneProprietaryOutput)
...
@Composable private fun GpsDataSourceCard(state:MainUiState,vm:MainViewModel){
 ...
 val nmeaReady=NmeaSourceSelectionPolicy.isUsablePosition(state.connection,state.nmeaFix,state.nmeaConnectionStartedElapsed,android.os.SystemClock.elapsedRealtime(),state.settings.gpsLossSeconds*1_000L)&&!state.outputSettings.phonePositionEnabled
