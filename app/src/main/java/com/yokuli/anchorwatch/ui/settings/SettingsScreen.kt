...
@Composable private fun PhoneVesselSensorsCard(state:MainUiState,vm:MainViewModel){
 ...
 Card{Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
  ...
  HorizontalDivider()
  Surface(color=if(productionReady)MaterialTheme.colorScheme.primaryContainer else MaterialTheme.colorScheme.errorContainer,shape=MaterialTheme.shapes.medium,modifier=Modifier.testTag("phone_sensor_readiness")){Column(Modifier.fillMaxWidth().padding(12.dp),verticalArrangement=Arrangement.spacedBy(4.dp)){Text(if(productionReady)tr("READY AS VESSEL SENSOR","可作为船舶传感器")else tr("NOT READY FOR PRODUCTION SHARING","尚不能用于生产分享"),fontWeight=FontWeight.Bold);Text(if(productionReady)tr("Trip motion and the canonical NMEA feed may use this calibrated vessel frame.","航程运动数据和统一 NMEA 数据流可以使用这个已校准的船体坐标系。")else tr("Complete every numbered step. Endpoint Test remains available on Data → Output, but Start sharing is blocked.","请完成全部编号步骤。数据 → 输出中的端点测试仍可用，但正式“开始分享”会被阻止。"),style=MaterialTheme.typography.bodySmall)}}
...
@Composable internal fun DataOutputSettingsPage(state:MainUiState,vm:MainViewModel){
 ...
 var showAdvancedTransport by rememberSaveable{mutableStateOf(state.outputSettings.transportMode!=NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION)}
 ...
 LazyColumn(Modifier.fillMaxSize().testTag("nmea_output_list").padding(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
  ...
  item{Card{Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   ...
   Text(if(state.outputSettings.transportMode==NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION)tr("Same-socket mode injects only fresh local Phone/App sensor evidence. Boat input, App TX echoes, and anything derived from Boat input are blocked from returning to this socket.","同 Socket 模式只注入新鲜的手机 / 本地 App 传感器数据。船载输入、App 发送回显，以及任何依赖船载输入的推算值都禁止写回此 Socket。")else tr("Independent output is a unified Anchor Watch vessel feed. Each metric follows the source selected in Data → Vessel and is re-encoded for separate clients.","独立输出是 Anchor Watch 统一船舶数据流。每项数值跟随“数据 → 船舶”选定的来源，并重新编码给独立客户端。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
   ...
   Text(if(state.outputSettings.transportMode==NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION)tr("Local injection: one atomic Phone GNSS fix produces RMC/GGA/VTG/ZDA; calibrated Phone heading produces HDT/optional HDG; Phone IMU produces ROT and heel/pitch XDR; Phone barometer produces pressure. Boat depth, STW and Boat-derived wind are never re-injected.","本地注入：同一次手机 GNSS 定位生成 RMC/GGA/VTG/ZDA；已校准手机方位生成 HDT / 可选 HDG；手机 IMU 生成 ROT 与横倾/纵倾 XDR；手机气压计生成气压。船载水深、对水航速及依赖船载数据的推算风绝不回灌。")else tr("Unified feed: selected Position, Heading, motion, pressure, coherent wind, depth and STW are re-encoded for an independent destination. Source selection remains in Data → Vessel.","统一数据流：将选定的船位、船首向、运动、气压、一致的风、水深和对水航速重新编码到独立目的地；来源仍在“数据 → 船舶”中选择。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
  ...
  item{Card(Modifier.testTag("nmea_output_route")){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
   ...
   Surface(color=MaterialTheme.colorScheme.surfaceVariant,shape=MaterialTheme.shapes.small){Column(Modifier.fillMaxWidth().padding(10.dp),verticalArrangement=Arrangement.spacedBy(5.dp)){
    ...
    val outputEndpoint=when(state.outputSettings.transportMode){NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION->state.settings.profile.host to state.settings.profile.port;NmeaOutputTransportMode.TCP_SERVER->"0.0.0.0" to state.outputSettings.outputPort;else->state.outputSettings.outputHost to state.outputSettings.outputPort}
   ...
   if(showAdvancedTransport){
    Text(tr("Only choose an independent route when the gateway documents a separate receive port, or when another device connects to this phone. UDP is connectionless, so a successful send cannot prove receiver consumption.","仅当网关明确提供独立接收端口，或其他设备需要连接本手机时，才选择独立线路。UDP 没有连接确认，发送成功不能证明接收端已经处理。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.tertiary)
   ...
   Text(when(routeMode){NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION->tr("The connected input endpoint must support full-duplex TCP. Output cannot start until that connection is live and writable.","已连接的输入端点必须支持全双工 TCP；连接可写之前不会启动输出。");NmeaOutputTransportMode.DEDICATED_TCP->tr("Advanced: the App connects to the gateway's documented separate receive port. This creates another TCP connection.","高级：App 连接网关明确提供的独立接收端口，并会新建另一条 TCP 连接。");NmeaOutputTransportMode.TCP_SERVER->tr("Advanced: the App listens on this phone; chartplotters or tablets connect as clients and receive the same canonical feed.","高级：App 在本手机监听；海图仪或平板作为客户端连接，并接收同一份统一数据流。");NmeaOutputTransportMode.UDP_UNICAST,NmeaOutputTransportMode.UDP_BROADCAST->tr("Advanced: UDP output is independent from NMEA input and has no receiver acknowledgement.","高级：UDP 输出与 NMEA 输入相互独立，并且没有接收确认。")},style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
   ...
   Text(if(routeMode==NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION)tr("Provenance firewall: Boat input is neither forwarded nor reformatted back to this socket. Derived values are allowed only when every dependency is local. Recently transmitted echoes are quarantined separately.","来源防火墙：船载输入既不会原样转发，也不会改写后回灌此 Socket。只有全部依赖均为本地来源的推算值才允许发送；最近发送的回显另行隔离。")else tr("This independent destination may receive the unified selected vessel state. Recently transmitted echoes are still quarantined from becoming new Boat evidence.","此独立目的地可以接收统一选定的船舶状态；最近发送的回显仍会被隔离，不能成为新的船载证据。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
  ...
  item{Card(Modifier.testTag("nmea_output_live_status")){Column(Modifier.fillMaxWidth().padding(14.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   ...
   if(state.outputSettings.transportMode==NmeaOutputTransportMode.TCP_SERVER){Text(tr("${state.nmeaSharing.clientCount} connected clients","${state.nmeaSharing.clientCount} 个客户端已连接"),style=MaterialTheme.typography.labelSmall);state.nmeaSharing.addresses.forEach{address->Text("tcp://${if(':' in address)"[$address]" else address}:${state.outputSettings.outputPort}",style=MaterialTheme.typography.labelSmall,fontFamily=FontFamily.Monospace)}}
   ...
   if(showPublisherDiagnostics){
    state.phonePositionOutputStatus.streams.filterKeys{it in setOf("POSITION","HEADING","MOTION","PRESSURE","DERIVED_WIND","DEPTH","SPEED_THROUGH_WATER","CANONICAL_FEED")}.forEach{(stream,status)->
   ...
   Text(if(state.outputSettings.transportMode==NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION)tr("Same-socket output ignores Data → Vessel AUTO selection and publishes only eligible local phone evidence. External-source conflicts remain visible here but never put a healthy Phone stream into standby.","同 Socket 输出不会采用“数据 → 船舶”的 AUTO 选中结果，只发布符合条件的手机本地证据。外部来源冲突仍会显示在这里，但绝不会让健康的手机数据流进入备用状态。")else tr("Data → Vessel owns source arbitration for this independent unified feed. This page owns transport, explicit Start/Stop, publication generation, live status and TX history.","此独立统一数据流的来源由“数据 → 船舶”仲裁；本页负责传输、明确启停、发布代次、实时状态和 TX 历史。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
  }}}
...
@Composable private fun publisherStreamLabel(value:String)=when(value){"POSITION"->tr("Position","船位");"HEADING"->tr("Vessel heading","船首向");"MOTION"->tr("Vessel motion","船体运动");"PRESSURE"->tr("Pressure","气压");"DERIVED_WIND"->tr("Derived true wind","推算真风");"DEPTH"->tr("Depth","水深");"SPEED_THROUGH_WATER"->tr("Speed through water","对水航速");"CANONICAL_FEED"->tr("Canonical client feed","统一客户端流");else->tr("Sensor status","传感器状态")}
...
@Composable private fun publisherOwnershipLabel(value:com.yokuli.anchorwatch.domain.vessel.PublisherOwnershipState)=when(value){
 ...
 com.yokuli.anchorwatch.domain.vessel.PublisherOwnershipState.PHONE_ACTIVE->tr("Canonical source active","统一来源已启用")
