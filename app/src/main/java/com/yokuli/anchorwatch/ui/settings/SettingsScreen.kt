...
@Composable private fun MapDepthSettingsPage(state:MainUiState,vm:MainViewModel){
 ...
 Card{Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
  ...
  HorizontalDivider();SettingSwitch(tr("Personal sonar layer","个人声呐图层"),tr("Saved survey cells are viewable offline; live NMEA is required only to record new soundings","已保存的调查网格可离线查看；只有记录新测深点才要求实时 NMEA"),state.settings.sonarLayerEnabled,true){enabled->if(enabled&&!state.settings.sonarDisclaimerAccepted)showSonarDisclaimer=true else vm.setSonarLayerEnabled(enabled)}
