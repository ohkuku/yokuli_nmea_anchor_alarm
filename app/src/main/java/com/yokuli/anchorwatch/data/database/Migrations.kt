...
/** Final Anchorage Library normalization. */
object Migration20To21:Migration(20,21){
 override fun migrate(db:SupportSQLiteDatabase){
  db.execSQL("ALTER TABLE anchorage_personal_ratings RENAME TO anchorage_personal_assessments")
  // A legacy visitCount is an imported summary and already includes the
  // source session represented by the migrated immutable Visit. Cached count
  // therefore starts at zero; later visits are additional observations.
  db.execSQL("UPDATE anchorage_places SET visitCountCached=0 WHERE legacySavedAnchorageId IS NOT NULL")
