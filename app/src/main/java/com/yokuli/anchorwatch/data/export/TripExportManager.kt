...
class TripExportManager @Inject constructor(
    ...
    suspend fun aiZip(session:TripSessionEntity):File{
        ...
        return zip("AnchorWatch_AI_Trip_$date.zip"){
            ...
            text("data_quality.json",Gson().toJson(mapOf("reportQuality" to report?.quality?.name,"positionCoveragePercent" to report?.positionCoveragePercent,"depthCoveragePercent" to report?.depthCoveragePercent,"attitudeCoveragePercent" to report?.attitudeCoveragePercent,"windCoveragePercent" to report?.windCoveragePercent,"droppedSamples" to session.droppedSampleCount)))
    ...
    private suspend fun writeTripSamples(target:File,id:Long)=target.bufferedWriter().use{w->
        w.appendLine("timestamp_utc,timestamp_epoch_ms,latitude,longitude,position_source,position_quality,position_age_ms,sog_knots,cog_true_deg,heading_true_deg,heading_source,heading_age_ms,depth_m,depth_source,depth_age_ms,stw_knots,stw_source,stw_age_ms,true_wind_knots,true_wind_direction_deg,true_wind_angle_deg,apparent_wind_knots,apparent_wind_angle_deg,wind_source,wind_age_ms,heel_deg,pitch_deg,roll_rate_deg_s,pitch_rate_deg_s,yaw_rate_deg_s,motion_score,roll_period_s,roll_period_confidence,attitude_age_ms,pressure_hpa,pressure_age_ms,ukc_m,source_flags")
        forEachTripSample(id){s->w.row(Instant.ofEpochMilli(s.timestamp),s.timestamp,s.latitude,s.longitude,s.positionSource,s.positionQuality,s.positionAgeMillis,s.sogKnots,s.cogTrueDegrees,s.headingTrueDegrees,s.headingSource,s.headingAgeMillis,s.depthMeters,s.depthSource,s.depthAgeMillis,s.speedThroughWaterKnots,s.stwSource,s.stwAgeMillis,s.trueWindSpeedKnots,s.trueWindDirectionDegrees,s.trueWindAngleDegrees,s.apparentWindSpeedKnots,s.apparentWindAngleDegrees,s.windSource,s.windAgeMillis,s.heelDegrees,s.pitchDegrees,s.rollRateDegPerSec,s.pitchRateDegPerSec,s.yawRateDegPerSec,s.motionScore,s.rollPeriodSeconds,s.rollPeriodConfidence,s.attitudeAgeMillis,s.pressureHpa,s.pressureAgeMillis,s.ukcMeters,s.sourceFlags)}
    }
