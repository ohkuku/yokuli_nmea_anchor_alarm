...
class SettingsRepository(private val context: Context) {
    private object K {
        val name=stringPreferencesKey("name");val host=stringPreferencesKey("host");val port=intPreferencesKey("port");val protocol=stringPreferencesKey("protocol");val checksum=booleanPreferencesKey("checksum");val auto=booleanPreferencesKey("auto");val map=intPreferencesKey("map");val wifi=booleanPreferencesKey("wifi");val gpsSource=stringPreferencesKey("gps_source");val mock=booleanPreferencesKey("mock");val enhanced=booleanPreferencesKey("mock_enhanced");val hz=intPreferencesKey("mock_hz");val gpsLoss=intPreferencesKey("gps_loss");val snooze=intPreferencesKey("alarm_snooze_minutes");val demoMode=booleanPreferencesKey("developer_demo_mode");val demoScenario=stringPreferencesKey("demo_scenario");val demoSpeed=intPreferencesKey("demo_speed_multiplier");val language=stringPreferencesKey("app_language")
    }
    ...
    val settings=context.store.data.map { p ->
        ...
        val storedSource=p[K.gpsSource]?.let{runCatching{GpsDataSource.valueOf(it)}.getOrNull()}
            ?:if(p[K.host]!=null)GpsDataSource.NMEA else GpsDataSource.SYSTEM
        AppSettings(
            ...
            gpsDataSource=if(storedSource==GpsDataSource.DEMO&&!demoEnabled)GpsDataSource.SYSTEM else storedSource,
    ...
    suspend fun save(s:AppSettings)=context.store.edit { p ->
        val safeSource=if(s.gpsDataSource==GpsDataSource.DEMO&&!s.demoMode)GpsDataSource.SYSTEM else s.gpsDataSource
        p[K.name]=s.profile.name;p[K.host]=s.profile.host;p[K.port]=s.profile.port;p[K.protocol]=s.profile.protocol.name;p[K.checksum]=s.profile.requireChecksum;p[K.auto]=s.profile.autoReconnect;p[K.map]=s.mapType;p[K.wifi]=s.keepWifiAwake;p[K.gpsSource]=safeSource.name;p[K.mock]=s.mockEnabled;p[K.enhanced]=s.enhancedMock;p[K.hz]=s.mockHz;p[K.gpsLoss]=s.gpsLossSeconds;p[K.snooze]=s.alarmSnoozeMinutes.coerceIn(1,30);p[K.demoMode]=s.demoMode;p[K.demoScenario]=s.demoScenario.name;p[K.demoSpeed]=s.demoSpeedMultiplier.takeIf{it in listOf(1,2,5)}?:1;p[K.language]=s.appLanguage.name
    }
