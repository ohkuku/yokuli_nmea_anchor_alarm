...
@Singleton class NavigationRepository @Inject constructor(
 ...
 fun accept(line:String,requireChecksum:Boolean=true){
  ...
  u.position?.let{position->
   ...
   val merged=position.copy(sogKnots=sog.fresh(now),cogTrueDegrees=cog.fresh(now),headingTrueDegrees=freshHeading,headingMagneticDegrees=headingMag.fresh(now),cogReceivedElapsedRealtime=cog?.second?.takeIf{now-it<=10_000},headingReceivedElapsedRealtime=headingTrue?.second?.takeIf{freshHeading!=null},depthMeters=depth.fresh(now),hdop=freshHdop,fixQuality=freshQuality,satellites=freshSatellites,horizontalAccuracyMeters=freshHdop?.times(3.0)?.coerceIn(2.5,80.0),positionProvider=PositionProvider.NMEA,headingSource=if(freshHeading!=null)HeadingSource.NMEA_PHYSICAL else HeadingSource.NONE,headingQuality=if(freshHeading!=null)HeadingQuality.STABLE else HeadingQuality.UNAVAILABLE,windDirectionTrueDegrees=freshTrueDirection,windSpeedKnots=freshTrueSpeed?:freshApparentSpeed,apparentWindAngleDegrees=freshApparentAngle,trueWindAngleDegrees=freshTrueAngle,trueWindSpeedKnots=freshTrueSpeed,apparentWindSpeedKnots=freshApparentSpeed,headingSampleSequence=headingSampleSequence.takeIf{freshHeading!=null},windSampleSequence=windSnapshot.sampleSequence)
   _fix.value=merged
