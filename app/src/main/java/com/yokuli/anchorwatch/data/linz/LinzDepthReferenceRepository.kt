...
class LinzQueryThrottle(private val movementMeters:Double=12.0,private val intervalMillis:Long=60_000L){
...
object LinzFinalResultCachePolicy{
    const val MAX_REUSE_DISTANCE_METERS=15.0
    fun isNear(cached:LinzDepthCacheEntity,latitude:Double,longitude:Double):Boolean=
        AnchorGeometry.distanceMeters(cached.queriedLatitude,cached.queriedLongitude,latitude,longitude)<=MAX_REUSE_DISTANCE_METERS
    fun canReuseFresh(cached:LinzDepthCacheEntity,latitude:Double,longitude:Double,nowWall:Long,ttlMillis:Long):Boolean=
        nowWall>=cached.queriedAt&&nowWall-cached.queriedAt<=ttlMillis&&isNear(cached,latitude,longitude)
}
...
class LinzDepthReferenceRepository @Inject constructor(private val client:LinzWfsClient,private val cache:LinzDepthCacheDao){
    ...
    suspend fun refresh(latitude:Double,longitude:Double,nowWall:Long=System.currentTimeMillis(),nowElapsed:Long=SystemClock.elapsedRealtime())=mutex.withLock{
        ...
        val freshCacheReusable=cached!=null&&LinzFinalResultCachePolicy.canReuseFresh(cached,latitude,longitude,nowWall,CACHE_TTL_MILLIS)
        if(freshCacheReusable){_state.value=requireNotNull(cached).model(cached=true);_diagnostics.value=_diagnostics.value.copy(cacheHits=_diagnostics.value.cacheHits+1,lastQueryAt=nowWall,lastQueryLatitude=latitude,lastQueryLongitude=longitude,message="Nearby final-result cache hit");return@withLock}
        ...
        try{
            ...
            if(cacheNearby){_state.value=requireNotNull(cached).model(cached=true).copy(status=LinzDepthStatus.OFFLINE);_diagnostics.value=_diagnostics.value.copy(requests=_diagnostics.value.requests+3,message="Offline · nearby cached LINZ depth")}
