...
class PhonePositionNmeaOutputRuntime @Inject constructor(
    ...
    @Synchronized fun configure(value:Boolean)=configure(NmeaDeviceOutputSettings(phonePositionEnabled=value,transportConfigured=value))
    ...
    private fun publishPosition(configured:NmeaDeviceOutputSettings,snapshot:VesselDataSnapshot,now:Long){
        lastPositionPublish=now;val conflict=snapshot.conflicts[VesselMetricId.POSITION]?.active==true;val decision=positionGate.evaluate(configured.effectivePositionPolicy,externalFresh(snapshot,VesselMetricId.POSITION,now,5_000L),conflict,now);outputConnection.recordDecision("POSITION",configured.effectivePositionPolicy,decision,latestPosition?.let{now-it.receivedElapsedRealtime in 0L..3_000L}==true)
        if(!decision.publish)return;val sentences=latestPosition?.let{mux.phonePosition(it,now)}?:emptyList();if(sentences.isEmpty()){outputConnection.recordSuppressed("POSITION",NmeaSuppressionReason.PHONE_GPS_STALE.name);return};write("POSITION",sentences,now)
    }
    private fun publishHeading(configured:NmeaDeviceOutputSettings,snapshot:VesselDataSnapshot,now:Long){
        lastHeadingPublish=now;val conflict=snapshot.conflicts[VesselMetricId.HEADING_TRUE]?.active==true;val decision=headingGate.evaluate(configured.effectiveHeadingPolicy,externalFresh(snapshot,VesselMetricId.HEADING_TRUE,now,3_000L),conflict,now);val ready=mountState==PhoneVesselMountState.VESSEL_MOUNTED&&heading.receivedElapsedRealtime?.let{now-it in 0L..1_500L}==true;outputConnection.recordDecision("HEADING",configured.effectiveHeadingPolicy,decision,ready)
        ...
        if(heading.receivedElapsedRealtime?.let{now-it in 0L..1_500L}!=true){outputConnection.recordSuppressed("HEADING",NmeaSuppressionReason.PHONE_HEADING_STALE.name);return}
    ...
    private fun publishMotion(configured:NmeaDeviceOutputSettings,snapshot:VesselDataSnapshot,now:Long){
        lastMotionPublish=now;val external=externalFresh(snapshot,VesselMetricId.RATE_OF_TURN,now,3_000L)||externalFresh(snapshot,VesselMetricId.HEEL,now,3_000L)||externalFresh(snapshot,VesselMetricId.PITCH,now,3_000L);val decision=motionGate.evaluate(configured.effectiveMotionPolicy,external,false,now);val live=attitude.attitude.takeIf{mountState==PhoneVesselMountState.VESSEL_MOUNTED&&attitude.receivedElapsedRealtime?.let{now-it in 0L..1_500L}==true};outputConnection.recordDecision("MOTION",configured.effectiveMotionPolicy,decision,live!=null);if(!decision.publish)return
    ...
    private fun write(stream:String,sentences:List<String>,now:Long){
        ...
        val sequence=outputConnection.recordGenerated(stream,now)
    ...
    fun testOutput(value:NmeaDeviceOutputSettings=settings,input:ConnectionProfile=inputProfile):Boolean{val testConfiguration=value.copy(proprietaryStatusEnabled=true);outputConnection.configure(testConfiguration,input);val result=outputConnection.write(input,listOf(mux.diagnostic()),setOf("YOK"));outputConnection.configure(value,input);return result}
    fun testKnownGoodHdg(value:NmeaDeviceOutputSettings=settings,input:ConnectionProfile=inputProfile):Boolean{val testConfiguration=value.copy(proprietaryStatusEnabled=true);outputConnection.configure(testConfiguration,input);return try{repeat(5){index->if(!outputConnection.write(input,listOf(mux.diagnosticMagneticHeading(123.4)),setOf("HDG")))return false;if(index<4)Thread.sleep(500L)};true}finally{outputConnection.configure(value,input)}}
