...
class PhonePositionNmeaOutputRuntime @Inject constructor(
    ...
    private fun publishPosition(configured:NmeaDeviceOutputSettings,snapshot:VesselDataSnapshot,now:Long){
        lastPositionPublish=now;val conflict=snapshot.conflicts[VesselMetricId.POSITION]?.active==true;val decision=positionGate.evaluate(configured.effectivePositionPolicy,SelectedExternalSourcePresence.present(snapshot.position,now,5_000L,setOf("RMC","GGA","GLL")),conflict,now);val fresh=latestPosition?.let{it.valid&&it.positionProvider==com.yokuli.anchorwatch.domain.model.PositionProvider.ANDROID_GNSS&&now-it.receivedElapsedRealtime in 0L..POSITION_HOLD_MILLIS}==true;outputConnection.recordDecision("POSITION",configured.effectivePositionPolicy,decision,fresh,NmeaStreamReadinessPolicy.position(fresh))
        if(!decision.publish)return;if(!fresh){outputConnection.recordSuppressed("POSITION",NmeaSuppressionReason.PHONE_GPS_STALE.name);return};val sentences=latestPosition?.let{mux.phonePosition(it,now,POSITION_HOLD_MILLIS)}?:emptyList();if(sentences.isEmpty()){outputConnection.recordSuppressed("POSITION",NmeaSuppressionReason.PHONE_GPS_STALE.name);return};write("POSITION",sentences,now)
    }
    private fun publishHeading(configured:NmeaDeviceOutputSettings,snapshot:VesselDataSnapshot,now:Long){
        lastHeadingPublish=now;val conflict=snapshot.conflicts[VesselMetricId.HEADING_TRUE]?.active==true;val decision=headingGate.evaluate(configured.effectiveHeadingPolicy,SelectedExternalSourcePresence.present(snapshot.headingTrueDegrees,now,3_000L,setOf("HDT","HDG")),conflict,now);val fresh=heading.receivedElapsedRealtime?.let{now-it in 0L..HEADING_HOLD_MILLIS}==true;val readiness=NmeaStreamReadinessPolicy.heading(calibration.calibratedAt>0L,calibration.headingAligned,mountState==PhoneVesselMountState.VESSEL_MOUNTED,heading.declinationReferenceReady,fresh);val ready=readiness==com.yokuli.anchorwatch.domain.vessel.NmeaStreamReadiness.READY;outputConnection.recordDecision("HEADING",configured.effectiveHeadingPolicy,decision,ready,readiness)
        if(!decision.publish)return
        if(calibration.calibratedAt<=0L||!calibration.headingAligned||mountState!=PhoneVesselMountState.VESSEL_MOUNTED){outputConnection.recordSuppressed("HEADING",if(mountState==PhoneVesselMountState.MOUNT_SUSPECT)NmeaSuppressionReason.MOUNT_SUSPECT.name else if(!calibration.headingAligned)"HEADING_NOT_ALIGNED" else NmeaSuppressionReason.PHONE_NOT_MOUNTED.name);return}
    ...
    private fun publishMotion(configured:NmeaDeviceOutputSettings,snapshot:VesselDataSnapshot,now:Long){
        lastMotionPublish=now;val external=SelectedExternalSourcePresence.present(snapshot.rateOfTurnDegreesPerMinute,now,3_000L);val decision=motionGate.evaluate(configured.effectiveMotionPolicy,external,false,now);val live=attitude.attitude.takeIf{mountState==PhoneVesselMountState.VESSEL_MOUNTED&&attitude.receivedElapsedRealtime?.let{now-it in 0L..MOTION_HOLD_MILLIS}==true};val readiness=NmeaStreamReadinessPolicy.motion(calibration.calibratedAt>0L,mountState==PhoneVesselMountState.VESSEL_MOUNTED,live!=null);outputConnection.recordDecision("MOTION",configured.effectiveMotionPolicy,decision,live!=null,readiness);if(!decision.publish)return
    ...
    private fun publishCanonical(snapshot:VesselDataSnapshot,now:Long){
        lastCanonicalPublish=now;val sentences=mux.canonicalFeed(snapshot,now);val ready=sentences.isNotEmpty();outputConnection.recordDecision("CANONICAL_FEED",PublicationPolicy.ALWAYS,PublicationDecision(ready,if(ready)PublisherOwnershipState.PHONE_ACTIVE else PublisherOwnershipState.SUPPRESSED),ready,NmeaStreamReadinessPolicy.sensor(ready))
        if(ready)write("CANONICAL_FEED",sentences,now)else outputConnection.recordSuppressed("CANONICAL_FEED","NO_CANONICAL_DATA")
    }
    private fun phoneVesselTrueHeading()=heading.liveTrueHeadingDegrees?.let{normalize(it+calibration.headingAlignmentOffsetDegrees)}.takeIf{calibration.headingAligned&&mountState==PhoneVesselMountState.VESSEL_MOUNTED&&heading.declinationReferenceReady}
    private fun phoneVesselMagneticHeading()=heading.liveMagneticHeadingDegrees?.let{normalize(it+calibration.headingAlignmentOffsetDegrees)}.takeIf{calibration.headingAligned&&mountState==PhoneVesselMountState.VESSEL_MOUNTED}
